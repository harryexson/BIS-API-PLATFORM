...
1. [Recovery Objectives (RPO/RTO)](#1-recovery-objectives-rpoto)
2. [Critical Data Inventory](#2-critical-data-inventory)
...
4. [Recovery Procedures](#4-recovery-procedures)
   ...
   - 4.4 Worker Recovery
   - 4.5 Redis Recovery
   - 4.6 Webhook Recovery
   - 4.7 Queue Recovery
5. [Recovery Simulation](#5-recovery-simulation)
6. [Recovery Contact Matrix](#6-recovery-contact-matrix)
7. [Testing Cadence](#7-testing-cadence)
...
```bash
curl -sf https://api.company.com/health | jq '.database'
# Expected: {"status":"healthy","latencyMs":<100}
```
...
### Automated Recovery Simulation Tests

Recovery simulations are automated in `packages/workers/src/disaster-recovery.test.ts`. Run with:

```bash
# Unit-level DR simulations (no external dependencies)
npx vitest run packages/workers/src/disaster-recovery.test.ts

# Full DR simulation suite (includes integration tests)
npx vitest run --config vitest.config.integration.ts packages/workers/src/disaster-recovery.integration.test.ts
```
...
| # | Gap | Severity | Status |
|---|-----|----------|--------|
| 1 | No automated health check monitoring/alerting | HIGH | Open |
| 2 | Runbooks reference Prisma — must update to Drizzle | HIGH | Fixed (Phase 46) |
| 3 | No Redis persistence configured | MEDIUM | Open |
| 4 | `SECRET_ENCRYPTION_KEY` not in `.env.example` | HIGH | Open |
| 5 | No circuit breaker implementation | MEDIUM | Open |
| 6 | Architecture DR doc referenced wrong table names | HIGH | Fixed (Phase 46) |
...
| Category | Tables |
|----------|--------|
| Application/Auth | `applications`, `application_api_keys`, `application_permissions`, `tenants`, `tenant_application_links`, `users`, `roles`, `permissions` |
| Providers | `providers`, `provider_configs`, `provider_capabilities`, `provider_health` |
| Events | `events`, `audit_logs` |
...
- [Architecture DR](docs/architecture/DISASTER_RECOVERY.md) — Technical architecture view
- [Operations DR](docs/operations/DISASTER_RECOVERY.md) — Operational runbook view
- [Incident Response](docs/operations/INCIDENT_RESPONSE.md) — Incident lifecycle
- [Rollback Procedures](docs/operations/ROLLBACK.md) — Code & database rollback
- [Runbooks](docs/operations/RUNBOOKS.md) — Step-by-step operational procedures
