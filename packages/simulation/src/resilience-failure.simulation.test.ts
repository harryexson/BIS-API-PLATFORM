...
const ALL_PROVIDERS = ['signalhouse', 'infobip', 'futuresms', 'example-msg', 'africastalking', 'stripe', 'nmi', 'email'];
...
describe('R4 — all SMS providers offline: silent channel change (GAP)', () => {
  it('an SMS is silently routed over email when no SMS provider is available', async () => {
    const sms = ['signalhouse', 'infobip', 'futuresms', 'example-msg', 'africastalking'];
